...
const runCorpusGolden = (label, env = {}) => {
  ...
  if (result.status !== 0) {
    console.error(`forma-ocaml elaboration parity ${label} run failed`);
...
runCorpusGolden("lisp fallback", {
  FORMA_DISABLE_NATIVE_ELABORATION: "1",
});

console.log(
  "forma-ocaml elaboration parity ok (native fast paths match Lisp fallback on corpus golden)",
);
