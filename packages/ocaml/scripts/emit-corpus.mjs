...
if (hardFailure) {
  console.error(`forma-ocaml corpus emit failed: ${hardFailure.message}`);
  process.exit(1);
}

console.log(
  `forma-ocaml corpus emit report: loaded ${loaded}/${sources.length}, emitted ${emitted}/${sources.length}, declarations ${declarationCount}`,
);
